...
( export PATH=`pwd`/artic_bin/bin:`pwd`/scripts:$PATH; export LD_LIBRARY_PATH=`pwd`/artic_bin/lib/; export INTERARTIC_MAC_VAR=`pwd`/artic_bin/lib/; bin/python3.7m bin/celery worker -A main.celery -b redis://localhost:$REDIS_PORT/0 --result-backend redis://localhost:$REDIS_PORT/0 --concurrency=1 --loglevel=info &> $CELERY_LOG || die "Launching celery failed. See $CELERY_LOG" ) &
sleep 1
echo ""
